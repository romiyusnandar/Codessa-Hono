/**
 * Vendored from @pandatix/js-cvss v0.4.4 (MIT License, © Lucas TESSON),
 * https://github.com/pandatix/js-cvss — logic itself is unmodified from
 * dist/cvss40.js + dist/errors.js, only converted to TypeScript.
 *
 * Why vendored instead of depending on the npm package directly: the published
 * package's package.json has no "type": "module" while its dist/*.js files use
 * extensionless ESM imports (e.g. `import * as errors from './errors'`) — that
 * resolves under bundler-style loaders (tsx, Jest) but throws ERR_MODULE_NOT_FOUND
 * under plain Node ESM, which is exactly how this project's production build runs
 * (`node dist/index.js`). Verified directly, not assumed. Re-exporting the fixed
 * logic here avoids depending on a package that's broken for our runtime.
 */

export class InvalidMetric extends Error {
  version: string;
  metric: string;
  constructor(version: string, metric: string) {
...
export class InvalidMetricValue extends Error {
  ...
  metric: string;
  value: string;
  constructor(version: string, metric: string, value: string) {
...
const re =
  /^CVSS:4[.]0(\/AV:[NALP])(\/AC:[LH])(\/AT:[NP])(\/PR:[NLH])(\/UI:[NPA])(\/VC:[HLN])(\/VI:[HLN])(\/VA:[HLN])(\/SC:[HLN])(\/SI:[HLN])(\/SA:[HLN])(\/E:[XAPU])?(\/CR:[XHML])?(\/IR:[XHML])?(\/AR:[XHML])?(\/MAV:[XNALP])?(\/MAC:[XLH])?(\/MAT:[XNP])?(\/MPR:[XNLH])?(\/MUI:[XNPA])?(\/MVC:[XNLH])?(\/MVI:[XNLH])?(\/MVA:[XNLH])?(\/MSC:[XNLH])?(\/MSI:[XNLHS])?(\/MSA:[XNLHS])?(\/S:[XNP])?(\/AU:[XNY])?(\/R:[XAUI])?(\/V:[XDC])?(\/RE:[XLMH])?(\/U:(?:X|Clear|Green|Amber|Red))?$/g;

/** Implementation of the CVSS v4.0 specification (https://www.first.org/cvss/v4.0/specification-document). */
export class CVSS40 {
  private _metrics: Record<string, string>;
  ...
  private parse(vector: string): void {
    const matches = vector.matchAll(re).next().value;
  ...
  Vector(): string {
    ...
    for (const [om] of Object.entries(table23)) {
      ...
      if (metric == undefined || metric == "X") continue;
  ...
  Score(): number {
    ...
    if (eq3 == 1 && eq6 == 1) {
      ...
      eq3eq6nlm = Math.max(
        mvs[String(eq1) + String(eq2) + String(eq3 + 1) + String(eq4) + String(eq5) + String(eq6)],
        mvs[String(eq1) + String(eq2) + String(eq3) + String(eq4) + String(eq5) + String(eq6 + 1)]
      );
      lower++;
    }

    const msd = (nlm: number) => {
      const d = Math.abs(nlm - eqsv);
      return isNaN(d) ? 0 : d;
    };
      ...
      for (const eq2mx of highestSeverityVectors[2][eq2]) {
        for (const eq3eq6mx of highestSeverityVectors[3][eq3][eq6]) {
          for (const eq4mx of highestSeverityVectors[4][eq4]) {
            ...
            if (
              [avsvdst, prsvdst, uisvdst, acsvdst, atsvdst, vcsvdst, visvdst, vasvdst, scsvdst, sisvdst, sasvdst, crsvdst, irsvdst, arsvdst].some(
                (met) => met < 0
              )
            ) {
              continue;
  ...
  Nomenclature(): string {
    const isDefined = (metric: string) => this.Get(metric) != "X";
    const t = ["E"].some(isDefined);
    const e = ["CR", "IR", "AR", "MAV", "MAC", "MAT", "MPR", "MUI", "MVC", "MVI", "MVA", "MSC", "MSI", "MSA"].some(isDefined);
    if (t) return e ? "CVSS-BTE" : "CVSS-BT";
    if (e) return "CVSS-BE";
    return "CVSS-B";
  }

  private getReal(metric: string): string | undefined {
  ...
  private static getValue(partial: string, metric: string): string | undefined {
    const pts = partial.split("/");
    for (const pt of pts) {
      const kv = pt.split(":");
      if (kv[0] == metric) return kv[1];
    }
    return undefined;
  }

  private static roundup(score: number): number {
...
const table23: Record<string, string[]> = {
  AV: ["N", "A", "L", "P"],
  AC: ["L", "H"],
  AT: ["N", "P"],
  PR: ["N", "L", "H"],
...
const highestSeverityVectors: Record<number, Record<number, any>> = {
  1: {
    0: ["AV:N/PR:N/UI:N"],
...
const sevIdx: Record<string, string[]> = {
  AV: ["N", "A", "L", "P"],
  AC: ["L", "H"],
  AT: ["N", "P"],
  PR: ["N", "L", "H"],
...
const depth: Record<number, any> = {
  1: { 0: 0, 1: 3, 2: 4 },
  2: { 0: 0, 1: 1 },
  3: { 0: { 0: 6, 1: 5 }, 1: { 0: 7, 1: 7 }, 2: { 1: 9 } },
  4: { 0: 5, 1: 4, 2: 3 },
  5: { 0: 1, 1: 1, 2: 1 },
};

// MacroVectors maximum score given each EQuivalency set. Table 29 (FIRST spec).
const mvs: Record<string, number> = {
  "000000": 10,
  "000001": 9.9,
  "000010": 9.8,
  "000011": 9.5,
